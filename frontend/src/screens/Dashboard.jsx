...
const Dashboard = () => {
  ...
  const handleSearch = (e) => {
    ...
    const filtered = sysTasks.filter((t) =>
      t.title.toLowerCase().startsWith(e.target.value.toLowerCase())
    );
    ...
    if (filtered?.length) {
      ...
      setSearching({
        action: true,
        title: "Nof found any task with the given template",
      });
  ...
  const handleDeleteTask = async (_id) => {
    try {
      ...
      if (res) {
        const updatedList = sysTasks.filter(
          (t) => t._id.toString() !== res.toString()
        );
  ...
  return (
    <>
      ...
      <Box>
        ...
        >
          {Boolean(crrUser) && crrUser.isSysAdmin && (
          ...
          <div className="myContainer">
            {!!sysTasks?.length && !!filteredTasks.length ? (
              <SysTasksContainer
                sysTasks={filteredTasks}
                user={crrUser}
