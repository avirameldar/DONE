  ...
  return (
    ...
    >
      <CardHeader
        ...
        onClick={
          props.openTaskDetails ? () => props.openTaskDetails(item._id) : null
        }
        title={item.title}
      ...
      <CardActions disableSpacing sx={{ width: "100%" }}>
          ...
          {props.isDone && (
            <Tooltip placement="top" title="restore task">
              <SettingsBackupRestoreIcon
                onClick={() => props.handleRestoreTask(item._id)}
              />
            </Tooltip>
          )}
        </IconButton>
        ...
        {props.item.public && props.isDashboard && (
          <IconButton aria-label="share">
            <Tooltip placement="top" title="pull task">
              <CompareArrowsIcon onClick={props.pullTask} />
            </Tooltip>
          </IconButton>
        )}
        ...
        {props.item.public && props.isDashboard && props.isSysAdmin && (
          <React.Fragment>
            <IconButton aria-label="pull">
              <Tooltip placement="top" title="pull task">
                <EditIcon onClick={() => props.editTask(item._id)} />
              </Tooltip>
            </IconButton>
            <IconButton aria-label="delete">
              <Tooltip placement="top" title="delete task">
                <DeleteIcon onClick={() => props.deleteItem(item._id)} />
              </Tooltip>
            </IconButton>
          </React.Fragment>
        )}
        ...
        {!props.isDashboard && !props.isDone && (
          <React.Fragment>
            ...
            <IconButton aria-label="share">
              <Tooltip placement="top" title="task completed">
                <DoneIcon onClick={() => props.handleCompleteTask(item._id)} />
              </Tooltip>
            </IconButton>
          </React.Fragment>
        )}
